...
main(int argc, char *const *argv)
{
	...
	/*根据命令行参数，将对应标志位置位；将输入的命令行参数（字符串地址）保存到对应的全局变量中*/
	...
	/*初始化ngx_cycle_t结构体中的prefix，conf_prefix，conf_file等变量*/
    ...
    for (i = 1; i < argc; i++) {
        ...
        while (*p) {
            ...
            case 'p':
                ...
                return NGX_ERROR;
			/*使用-c参数指定配置文件来启动nginx，例如： /usr/local/nginx/sbin/nginx  -c  /tmp/nginx.conf  */
            case 'c':
...
ngx_save_argv(ngx_cycle_t *cycle, int argc, char *const *argv)
{
	...
	/*为argv指向的参数分配空间ngx_argv[i]，将argv保存到ngx_argv中*/
    for (i = 0; i < argc; i++) {
...
ngx_process_options(ngx_cycle_t *cycle)
{
	...
	if (ngx_prefix) {
		...
		/* 若p指向的内容不是以‘/’结尾，则为它分配内存，将ngx_prefix的内容复制过去,并结尾加上‘/’ */
	...
	// 将获得配置文件的绝对路径，并保存到cycle->conf_file
    ...
    for (p = cycle->conf_file.data + cycle->conf_file.len - 1;
         ...
         p--)
    {
        /* p指针由字符串尾向头部移动，遇到第一个‘/’，执行下面操作，实际是去掉配置文件名，conf_prefix只保存配置文件的目录 */
