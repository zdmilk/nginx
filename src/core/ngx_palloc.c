...
ngx_create_pool(size_t size, ngx_log_t *log)
{
    ...
    p->max = (size < NGX_MAX_ALLOC_FROM_POOL) ? size : NGX_MAX_ALLOC_FROM_POOL;  ////最大不超过4095B
...
ngx_palloc_small(ngx_pool_t *pool, size_t size, ngx_uint_t align)
{
    ...
    p = pool->current; // 小于max值，从current节点开始遍历pool链表
    ...
    do {
        ...
        if ((size_t) (p->d.end - m) >= size) {  //有足够的空间分配size大小的内存
            p->d.last = m + size;
...
ngx_palloc_block(ngx_pool_t *pool, size_t size)
{
    ...
    for (p = pool->current; p->d.next; p = p->d.next) {
        if (p->d.failed++ > 4) {
            pool->current = p->d.next;  //在数据区分配size大小的内存并设置last指针
        }
    }
